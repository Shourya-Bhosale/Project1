...
def _send_order_emails(order: Order) -> None:
    ...
    # Send confirmation email to customer (fail silently to prevent blocking)
    if order.email:
        try:
            ...
            if result:
                ...
                print(f"⚠️ Email sending returned False for customer: {order.email} - check email settings")
        except Exception as e:
            print(f"❌ Failed to send email to customer {order.email}: {str(e)}")
            import traceback
            print(traceback.format_exc())
    
    # Send notification email to company (fail silently to prevent blocking)
    if company_email:
        try:
            ...
            if result:
                ...
                print(f"⚠️ Email sending returned False for company: {company_email} - check email settings")
        except Exception as e:
